...
export function parsePDB(pdbText: string): Protein {
  const atoms: Atom[] = [];
  ...
  for (const atom of atoms) {
    ...
    if (!resMap.has(atom.resSeq)) {
      ...
      let ss: "helix" | "sheet" | "coil" | "turn" = "coil";
      if (helixResidues.has(key)) ss = "helix";
      else if (sheetResidues.has(key)) ss = "sheet";
